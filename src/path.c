...
struct dynamic_array *djikstra(struct graph_t *G, size_t M, int src, enum color_t id)
{
  ...
  for (int count = 0; count < (int)(G->num_vertices - 1); count++)
  {
    ...
    for (size_t v = 0; v < G->num_vertices; v++)
    {
      ...
      if (!sptSet[v] && gsl_spmatrix_get(G->t, u, v) && dist[u] != INFINIT && !gsl_spmatrix_get(G->o, (id + 1) % 2, v) && dist[u] + (int)(gsl_spmatrix_get(G->t, u, v) - gsl_spmatrix_get(G->o, id, v) * gsl_spmatrix_get(G->o, id, u)) <= dist[v])
      {
